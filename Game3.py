...
def Start3(win, N, C, U, abset, eqround, BigArray, BigArrayScore, HouseArray, Composition, NumAltruists_text, NumEgoists_text, Equilibrium_text, RoundNumber_text):
    ...
    HouseArray = House3_Draw(win, array, N, HouseArray)  ##draw the houses in the neighborhood
    ...
    eqround = Equilibrium_Round3(array, C, U, eqround)  ##find the equilibrium of the given neighborhood
    abset = AbsorbingSet3(array, scorearray, C, U, abset, eqround) ##find the number of sub absorbing set at equilibrium in the given neighborhood
...
def Important_Parameters3(win, N_Entry, C_Entry, U_Entry, Composition_Entry, HouseArray, warning_text2):
...
def Game3(win):
    ...
    ### Draw Game 3 ###
    grass_image, NumAltruists_text, NumEgoists_text, RoundNumber_text, warning_text1, Equilibrium_text, Absorbing_text, Instruction_Button, Previous_Button, Next_Button, Round_Entry, Reset_Button, Menu_Button, Exit_Button, marble_image, rect1, steeel_image, GameParameters_text, N_Entry, N_text, C_Entry, C_text, U_Entry, U_text, Composition_Entry, Composition_text, warning_text2, rect2, steel_image2, GameFunctions_text, Equilibrium_Button, AbsorbingSet_Button = Game3_Draw(win)

    ### Set Important Parameters ###
    HouseArray = [""]
    vertical = 0
    horizontal = 0
    N, C, U, Round, BigArray, BigArrayScore, Composition, HouseArray, eqround, abset = Important_Parameters3(win, N_Entry, C_Entry, U_Entry, Composition_Entry, HouseArray, warning_text2)

    ### Initializing the Game (Population + Calculations) ###
    array, scorearray, BigArray, BigArrayScore, HouseArray, eqround, abset = Start3(win, N, C, U, abset, eqround, BigArray, BigArrayScore, HouseArray, Composition, NumAltruists_text, NumEgoists_text, Equilibrium_text, RoundNumber_text)
    ...
    while(Menu_Button.isClicked(pt) == False):
        while(Reset_Button.isClicked(pt) == False):
            ...
            if(Next_Button.isClicked(pt)):
                array, BigArray, BigArrayScore, HouseArray, Round = Round_Next3(win, array, scorearray, N, C, U, Round, BigArray, BigArrayScore, HouseArray, NumAltruists_text, NumEgoists_text, RoundNumber_text, warning_text1, Round_Entry)
            ...
            ## Check if the previous button is clicked and if so it prints out the previous state of the neighborhood in the GUI ##
                ...
                array, BigArray, BigArrayScore, HouseArray, Round = Round_Previous3(win, array, scorearray, N, C, U, Round, BigArray, BigArrayScore, HouseArray, NumAltruists_text, NumEgoists_text, RoundNumber_text, warning_text1, Round_Entry)
    ...
    if(Menu_Button.isClicked(pt)):
        ...
        Game3_Undraw(grass_image, NumAltruists_text, NumEgoists_text, RoundNumber_text, warning_text1, Equilibrium_text, Absorbing_text, Instruction_Button, Previous_Button, Next_Button, Round_Entry, Reset_Button, Menu_Button, Exit_Button, marble_image, rect1, steeel_image, GameParameters_text, N_Entry, N_text, C_Entry, C_text, U_Entry, U_text, Composition_Entry, Composition_text, warning_text2, rect2, steel_image2, GameFunctions_text, Equilibrium_Button, AbsorbingSet_Button, HouseArray)
    if(Exit_Button.isClicked(pt)):
        ## Closes and ends the program ##
        win.close()
        sys.exit()
