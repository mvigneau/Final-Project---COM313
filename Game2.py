...
##File Description:
##
##    The file is used as a "main" for Game 2 as it takes care of all the logical aspect while inside Game 2.
##    The file also uses the Game1_Function file for calling various functions and the Game2_objects to add objects to the window etc.
##    The file is linked to the actual main function where it is called when Game 2 is being played.
#################################################
...
from DeterministicAlgorithms import *
...
def Start2(win, N, C, U, abset, eqround, BigArray, BigArrayScore, HouseArray, Composition, NumAltruists_text, NumEgoists_text, Equilibrium_text, RoundNumber_text):
    ...
    eqround = Equilibrium_Round2(array, C, U, eqround) ##find the equilibrium of the given neighborhood
    abset = AbsorbingSet2(array, scorearray, N, C, U, abset, eqround) ##find the number of sub absorbing set at equilibrium in the given neighborhood
...
def Game2(win):
    ...
    array, scorearray, BigArray, BigArrayScore, HouseArray, eqround, abset = Start2(win, N, C, U, abset, eqround, BigArray, BigArrayScore, HouseArray, Composition, NumAltruists_text, NumEgoists_text, Equilibrium_text, RoundNumber_text)
    ...
    while(Menu_Button.isClicked(pt) == False):
        while(Reset_Button.isClicked(pt) == False):
            ...
            if(Next_Button.isClicked(pt)):
                array, BigArray, BigArrayScore, HouseArray, Round = Round_Next2(win, array, scorearray, N, C, U, Round, BigArray, BigArrayScore, HouseArray, NumAltruists_text, NumEgoists_text, RoundNumber_text, warning_text1, Round_Entry)
